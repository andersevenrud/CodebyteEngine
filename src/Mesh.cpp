...
Mesh* Mesh::At(ulong id){
    for(uint i = 0; i < (uint)meshes.size(); i++){
        if(meshes.at(i)->id == id) return meshes.at(i);
    }
    return NULL;
}
Mesh* Mesh::At(String name){
    for(uint i = 0; i < (uint)meshes.size(); i++){
        if(meshes.at(i)->name == name) return meshes.at(i);
    }
    std::cout << "Mesh with name: " << name << " does not exist!" << std::endl;
    return NULL;
}
Mesh* Mesh::WithLocation(String location){
    for(uint i = 0; i < (uint)meshes.size(); i++){
        if(meshes.at(i)->fileLocation == location) return meshes.at(i);
    }
    return NULL;
}
void Mesh::Cleanup(){
