...
void Setup(){
    ...
    player = Node::At("player");
    playerCamera = Node::At("playerCamera");
    boat = Node::At("boat");
    Camera::SetCurrentCamera(camera, &playerCamera->transform);
...
static void onresize(GLFWwindow* window, int width, int height){
    ...
    GUI::Manager::Resize(width, height);
    Camera::GetCurrentCamera()->SetAspect((float)width / (float)height);
}
