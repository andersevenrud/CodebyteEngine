...
/********************** Static code **********************/
ulong Shader::nextID = 0;
std::vector<Shader*> Shader::shaders;
Shader* Shader::At(ulong id){
    for(uint i = 0; i < (uint)shaders.size(); i++){
        if(shaders.at(i)->id == id) return shaders.at(i);
    }
    return NULL;
}
Shader* Shader::At(String name){
    for(uint i = 0; i < (uint)shaders.size(); i++){
        if(shaders.at(i)->name == name) return shaders.at(i);
    }
    std::cout << "Shader with name: " << name << " does not exist!" << std::endl;
    return NULL;
}
Shader* Shader::WithLocation(String location){
    for(uint i = 0; i < (uint)shaders.size(); i++){
        if(shaders.at(i)->fileLocation == location) return shaders.at(i);
    }
    return NULL;
}
void Shader::Cleanup(){
...
bool Shader::Load(String location){
	...
	if(this->shaderID == 0){
		...
		if(file.is_open()){
			...
			std::cout << "Unable to load shader: " << fileLocation << std::endl;
		}
	}else{
		std::cout << "Shader aleady loaded!" << fileLocation << std::endl;
	}
...
GLuint Shader::GetUniform(String name){
	...
	if(it == this->uniforms.end()){
        std::cout << "Uniform '" << name << "' does not exist in shader " << this->name << "!" << std::endl;
		return -1;
	}else{
		return it->second;
	}
