...
/********************** Static code **********************/
ulong Material::nextID = 0;
std::vector<Material*> Material::materials;
Material* Material::At(ulong id){
    for(uint i = 0; i < (uint)materials.size(); i++){
        if(materials.at(i)->id == id) return materials.at(i);
    }
    return NULL;
}
Material* Material::At(String name){
    for(uint i = 0; i < (uint)materials.size(); i++){
        if(materials.at(i)->name == name) return materials.at(i);
    }
    std::cout << "Material with name: " << name << " does not exist!" << std::endl;
    return NULL;
}
Material* Material::WithLocation(String location){
    for(uint i = 0; i < (uint)materials.size(); i++){
        if(materials.at(i)->fileLocation == location) return materials.at(i);
    }
    return NULL;
}
...
void Material::Load(String location){
    ...
    for(uint i = 0; i < (uint)contents.size(); i++){
        ...
        if(i == 0){
            if(line.substr(0, 1) == "["){
                this->shader = Shader::At(StringStrip(line, "[]"));
                if(this->shader == NULL){
            ...
            if((uint)tokens.size() == 3){
                ...
                if(tokens.at(0) == "float"){
                    ...
                    Texture * texture = Texture::WithLocation(tokens.at(2));
                    if(texture != NULL)
